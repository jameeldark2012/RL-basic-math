...
class TabularEnv(gym.Env):
    ...
    def reset(self , seed =0):
        ...
        return np.array(observation["Reward"] , dtype=np.float32) , {}
    ...
    def step(self , action):
        ...
        return np.array(self.table["Reward"] , dtype=np.float32)  ,reward, done_env, truncated, info
    ...
    # Find the matching row
        ...
        if float(match) >0:
            ...
            if bool(match == max(self.table.iloc[:]["Reward"])):
...
#         logger = configure("logs\\PPODISC", ["stdout", "csv", "tensorboard"])
#         model_params = {
#         "policy":"MlpPolicy",
#         "gamma":0.1,
#  } 
